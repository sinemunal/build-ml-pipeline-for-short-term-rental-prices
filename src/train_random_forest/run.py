...
def go(args):
    ...
    ######################################
    logging.info("Get train and validation data")
    ...
    ######################################
    logging.info("Save the pipeline as a mlflow.sklearn model")
    # create temporary predictions for schema inference
    y_pred_nonan = sk_pipe.predict(X_val.dropna())
    mlflow.sklearn.save_model(
        sk_pipe,
        "random_forest_dir",
        serialization_format=mlflow.sklearn.SERIALIZATION_FORMAT_CLOUDPICKLE,
        signature=infer_signature(X_val.dropna(), y_pred_nonan),
        input_example=X_val.iloc[:2],
    )
    ######################################

    ######################################
    logging.info("Upload the exported model")
    ...
    ######################################
    logging.info("Upload performance params and feature importances")
